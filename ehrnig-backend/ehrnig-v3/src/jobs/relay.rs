...
async fn run_relay_process(db: &DbPool, mq: &MqPool) -> anyhow::Result<()> {
    ...
    for ev in events {
        match map_event_to_job(&ev) {
            // Found mapping(registered events), publish to the queue for consumer workers to consume
            Ok(Some(job)) => {
            ...
            // Found unregistered event.
            Ok(None) => {
                // we mark it processed so we don't look at it again
                mark_as_processed(db, ev.id).await?;
            }
...
/*
   INFO: FUTURE IMPLEMENTATION: will moved this to the workers (consumer) in future, this is because the outbox events are supposed
   to be marked as processed after the consumer acknowledge success not when they are publish to the queue
*/
...
// Register the events and map to the specific job Enum
