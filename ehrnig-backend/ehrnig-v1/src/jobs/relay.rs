...
async fn run_relay_process(db: &DbPool, mq: &MqPool) -> anyhow::Result<()> {
    ...
    for ev in events {
        // STEP: Convert specific DB event to generic Job Enum
        let job = match map_event_to_job(&ev) {
            Ok(Some(j)) => j,
            _ => continue,
        };

        // Publish to a queue named after the event_type (e.g., "patient.created")
        if let Ok(_) = publish_event(mq, constants::JOB_QUEUE_NAME, &job).await {
            // Mark as processed ONLY if RabbitMQ accepted it
            sqlx::query!(
