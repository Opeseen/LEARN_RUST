...
pub async fn create_organization(
    ...
    let organization_id = OrganizationStorage::create(&state.db, &payload)
        .await
        .map_err(|e| match e {
...
pub async fn get_all_organization(
    ...
    let organizations = OrganizationStorage::find_all(&state.db)
        .await
        .map_err(|_| {
            ApiResponse::error(StatusCode::INTERNAL_SERVER_ERROR, constants::INTERNAL_ERROR)
        })?;
...
pub async fn get_organization(
    ...
    let organizations = OrganizationStorage::find_by_id(&state.db, id)
        .await
        .map_err(|_| {
            ApiResponse::error(StatusCode::INTERNAL_SERVER_ERROR, constants::INTERNAL_ERROR)
        })?;
