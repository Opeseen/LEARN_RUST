...
use infrastructures::{config::Config, db, email::EmailService, mq, redis};
...
async fn main() {
    ...
    let redis_client = redis::create_redis_client(config.redis_url).await;
    ...
    tokio::spawn(async move {
        workers::start_workers(mq_pool, email_service).await;
    });
